...
execute if score #game GameState matches 0 as @a[scores={Trig_GameStart=1..}] run scoreboard players set #game Timer 1200
...
execute if score #game GameState matches 2 as @a[scores={Deaths=1..}] run tellraw @a ["",{"text":"There are now ","color":"dark_red"},{"score":{"name":"#game","objective":"PlayersAlive"},"bold":true,"color":"dark_red"},{"text":" players left in the match","color":"dark_red"}]
