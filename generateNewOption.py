def main():
    ...
    if (defaultActive == "y"):
        ...
        print("/give @p minecraft:oak_sign{{BlockEntityTag:{{Text1:\'{{\"text\":\"\",\"clickEvent\":{{\"action\":\"run_command\",\"value\":\"/trigger Trig_{0} add 1\"}}}}\',Text2:\'{{\"text\":\"{1}\",\"clickEvent\":{{\"action\":\"run_command\",\"value\":\"/execute as @p[scores={{Trig_{0}=1..}}] if score #game {0} matches 0 run data merge block ~ ~ ~ {{Text3:\\\'{{\\\\\"color\\\\\":\\\\\"green\\\\\", \\\\\"text\\\\\":\\\\\"[Enabled]\\\\\"}}\\\'}}\"}}}}\',Text3:\'{{\"color\":\"red\",\"text\":\"[Disabled]\"}}\',Text4:\'{{\"text\":\"\",\"clickEvent\":{{\"action\":\"run_command\",\"value\":\"/execute as @p[scores={{Trig_{0}=1..}}] if score #game {0} matches 1 run data merge block ~ ~ ~ {{Text3:\\\'{{\\\\\"color\\\\\":\\\\\"red\\\\\",\\\\\"text\\\\\":\\\\\"[Disabled]\\\\\"}}\\\'}}\"}}}}\'}},display:{{Name:\'{{\"text\":\"Custom Sign\"}}\'}}}}".format(optionName, signText))
